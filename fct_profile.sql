...
CREATE INDEX ix_u
ON fct_xdr (MSISDN,IAB_TIER1,DOWNLOAD_BYTES,HOST);

INSERT INTO fct_profile
SELECT B.MSISDN, CONCAT(FLOOR(rand_mt()*450)+400,"|",FLOOR(rand_mt()*246000),"|",FLOOR(RAND()*254233),"|", TRUNCATE(RAND(),4)) AS PROFILE 
FROM (SELECT DISTINCT A.MSISDN FROM src_xdr A WHERE A.IAB_TIER1 IN ('ACCESSORIES','ACCOUNTING','ADD','ADULT_EDUCATION','ARTS_CRAFTS','BARBECUES_GRILLING','CHRISTIANITY','EDUCATIONAL_INSTITUTIONS','ENTERTAINMENT_NEWS_CELEBRITY_SITES','ENTERTAINMENT_OTHER','FINANCIAL_PLANNING','HEALTH_LOWFAT_COOKING','INVESTING','LITERATURE_BOOKS','MOVIES','MUSIC','PRIVATE_SCHOOL','PSYCHOLOGY_PSYCHIATRY','REFERENCE_MATERIALS_MAPS','SMOKING_CESSATION','SPACE_ASTRONOMY','SPECIAL_EDUCATION','STREAMING_DOWNLOADABLE_VIDEO','TAX_PLANNING','TELEVISION','TEXT_MESSAGING_SMS','WIKIS','YEAR_712_EDUCATION') ) B

INSERT INTO fct_control
SELECT DISTINCT MSISDN, IAB_TIER1 FROM src_xdr
