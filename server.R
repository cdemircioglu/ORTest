...
function(input, output, session) {
  ...
  output$packagePlot <- renderBubbles({
    ...
    df <- pkgData() %>%
      ...
      bubbles(df$cmsisdn, paste("$",floor((df$size/4)-input$costtoDeliver), "/", df$cmsisdn/100,"K",sep="" ), key = df$size, color = c(cp(nrow(df[which(df$size>=0),])),rev(cn(nrow(df[which(df$size<0),])))) )
