...
function(input, output, session) {
  ...
  output$totalcustomersScanned <- renderValueBox({
    ...
    if(exists("df"))
    {
      valueBox(
        #value = customerCount(),
        value = paste(prettyNum(floor(df$cmsisdn/200), scientific=FALSE, big.mark=','),"K",sep=""),
        #value = df$cmsisdn,
  ...
  output$packagePlot <- renderBubbles({
    ...
    df <- pkgData() %>%
      ...
      bubbles(df$cmsisdn, paste("$",floor((df$size)-input$costtoDeliver), "/", df$cmsisdn/100,"K",sep="" ), key = df$size, color = c(cp(nrow(df[which(floor((df$size)-input$costtoDeliver)>=0),])),rev(cn(nrow(df[which(floor((df$size)-input$costtoDeliver)<0),])))) )
